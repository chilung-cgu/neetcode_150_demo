...
| We solve Maximum Subarray. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Kadane's Algorithm (Greedy) as main method. | 我會用來源主方法。 | Restatement |
...
| Brute space is about O(N^3) from source. | 來源暴力空間約 O(N^3)。 | Approach |

### Optimized approach (5 lines)

| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Kadane's Algorithm (Greedy). | 優化法使用來源主方法。 | Approach |
...
| Then, I update state for each element. | 依來源步驟執行。 | Coding |
| Next, I check condition and update answer. | 依來源步驟執行。 | Coding |
| Then, I continue until iteration completes. | 依來源步驟執行。 | Coding |
| Next, I return the final computed result. | 依來源步驟執行。 | Coding |
...
| Sample input: nums = [-2,1,-3,4,-1,2,1,-5,4]. | 範例輸入：nums = [-2,1,-3,4,-1,2,1,-5,4]。 | Dry-run |
