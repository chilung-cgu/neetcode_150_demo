...
| We solve Partition Labels. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Greedy + Last Index Map as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Greedy + Last Index Map. | 優化法使用來源主方法。 | Approach |
...
| Sample input: s = "ababcbacadefegdehijhklij". | 範例輸入：s = "ababcbacadefegdehijhklij"。 | Dry-run |
...
| Expected output: [9,7,8]. | 預期輸出：[9,7,8]。 | Dry-run |
