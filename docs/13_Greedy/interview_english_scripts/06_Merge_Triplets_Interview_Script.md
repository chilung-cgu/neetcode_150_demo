# 06 Merge Triplets to Form Target Triplet — Interview English Script (C++)
...
| We solve Merge Triplets to Form Target Triplet. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Greedy Filtering as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Greedy Filtering. | 優化法使用來源主方法。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| First, I initialize required data structures. | 依來源步驟執行。 | Coding |
| Next, I iterate input following source order. | 依來源步驟執行。 | Coding |
| Then, I use unordered_set for membership checks. | 依來源步驟執行。 | Coding |
| Next, I update state for each element. | 依來源步驟執行。 | Coding |
| Then, I check condition and update answer. | 依來源步驟執行。 | Coding |
| Next, I continue until iteration completes. | 依來源步驟執行。 | Coding |
| Then, I return the final computed result. | 依來源步驟執行。 | Coding |
...
| Sample input: triplets = [[2,5,3],[1,8,4],[1,7,5]], target = [2,7,5]. | 範例輸入：triplets = [[2,5,3],[1,8,4],[1,7,5]], target = [2,7,5]。 | Dry-run |
