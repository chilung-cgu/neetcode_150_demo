...
| We solve Valid Parenthesis String. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Greedy Range as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(3^N) from source. | 來源暴力時間約 O(3^N)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Greedy Range. | 優化法使用來源主方法。 | Approach |
