...
| We solve Burst Balloons. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Interval DP as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Interval DP. | 優化法使用來源主方法。 | Approach |
...
| Sample input: nums = [3,1,5,8]. | 範例輸入：nums = [3,1,5,8]。 | Dry-run |
...
| Expected output: 167. | 預期輸出：167。 | Dry-run |
...
| Time is O(N^3), space is O(N^2). | 收尾時可用這句。 | Wrap-up |
