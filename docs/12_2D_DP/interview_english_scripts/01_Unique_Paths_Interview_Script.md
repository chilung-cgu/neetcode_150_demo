...
| We solve Unique Paths. | 我們要解這一題。 | Restatement |
...
| Brute time is about O(2^{M+N}) from source. | 來源暴力時間約 O(2^{M+N})。 | Approach |
...
| Sample input: m = 3, n = 7. | 範例輸入：m = 3, n = 7。 | Dry-run |
...
| Expected output: 28. | 預期輸出：28。 | Dry-run |
