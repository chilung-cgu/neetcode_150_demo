...
| We solve Distinct Subsequences. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use 2D DP (Standard) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses 2D DP (Standard). | 優化法使用來源主方法。 | Approach |
...
| Sample input: s = "rabbbit", t = "rabbit". | 範例輸入：s = "rabbbit", t = "rabbit"。 | Dry-run |
...
| Time is O(M \times N), space is O(N). | 收尾時可用這句。 | Wrap-up |
