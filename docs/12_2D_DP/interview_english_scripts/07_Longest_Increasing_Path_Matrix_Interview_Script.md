# 07 Longest Increasing Path in a Matrix — Interview English Script (C++)
...
| We solve Longest Increasing Path in a Matrix. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use DFS + Memoization as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(4^{M \times N}) from source. | 來源暴力時間約 O(4^{M \times N})。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses DFS + Memoization. | 優化法使用來源主方法。 | Approach |
