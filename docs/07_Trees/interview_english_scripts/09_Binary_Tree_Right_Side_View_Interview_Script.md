...
| We solve Binary Tree Right Side View. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Iterative BFS (Queue) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Iterative BFS (Queue). | 優化法使用來源主方法。 | Approach |
...
| Sample input: root = [1,2,3,null,5,null,4]. | 範例輸入：root = [1,2,3,null,5,null,4]。 | Dry-run |
...
| Expected output: [1,3,4]. | 預期輸出：[1,3,4]。 | Dry-run |
