...
| We solve Subtree of Another Tree. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Check Same Tree for Every Node as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Check Same Tree for Every Node. | 優化法使用來源主方法。 | Approach |
...
| Optimized space is O(M) from source. | 來源優化空間為 O(M)。 | Approach |
...
| Space complexity is O(M). | 空間複雜度是 O(M)。 | Complexity |
...
| Extra memory from source is O(M). | 來源額外空間為 O(M)。 | Complexity |
...
| Time is O(M \times N), space is O(M). | 收尾時可用這句。 | Wrap-up |
...
| Report space complexity O(M). | 速記重點。 | Cheat sheet |
