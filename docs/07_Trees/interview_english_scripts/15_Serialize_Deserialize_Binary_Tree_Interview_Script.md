...
| We solve Serialize and Deserialize Binary Tree. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Preorder DFS as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Preorder DFS. | 優化法使用來源主方法。 | Approach |
...
| Sample input: root = [1,2,3,null,null,4,5]. | 範例輸入：root = [1,2,3,null,null,4,5]。 | Dry-run |
...
| Expected output: Same tree object.. | 預期輸出：Same tree object.。 | Dry-run |
