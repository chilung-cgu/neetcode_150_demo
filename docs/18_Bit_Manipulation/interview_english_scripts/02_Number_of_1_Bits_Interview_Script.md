...
| We solve Number of 1 Bits. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Brian Kernighan's Algorithm as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(32) from source. | 來源暴力時間約 O(32)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Brian Kernighan's Algorithm. | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(k) from source. | 來源優化時間為 O(k)。 | Approach |
...
| Sample input: n = 00000000000000000000000000001011. | 範例輸入：n = 00000000000000000000000000001011。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(k). | 時間複雜度是 O(k)。 | Complexity |
...
| Main runtime from source is O(k). | 來源主要時間為 O(k)。 | Complexity |
...
| Time is O(k), space is O(1). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(k). | 速記重點。 | Cheat sheet |
