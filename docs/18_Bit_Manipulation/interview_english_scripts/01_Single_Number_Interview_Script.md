...
| We solve Single Number. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Bit Manipulation (XOR) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Bit Manipulation (XOR). | 優化法使用來源主方法。 | Approach |
...
| Sample input: [2,2,1]. | 範例輸入：[2,2,1]。 | Dry-run |
