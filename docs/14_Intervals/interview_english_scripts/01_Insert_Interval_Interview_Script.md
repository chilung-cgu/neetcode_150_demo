...
| We solve Insert Interval. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use One Pass (Greedy) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses One Pass (Greedy). | 優化法使用來源主方法。 | Approach |
...
| Sample input: intervals = [[1,3],[6,9]], newInterval = [2,5]. | 範例輸入：intervals = [[1,3],[6,9]], newInterval = [2,5]。 | Dry-run |
...
| Expected output: [[1,5],[6,9]]. | 預期輸出：[[1,5],[6,9]]。 | Dry-run |
