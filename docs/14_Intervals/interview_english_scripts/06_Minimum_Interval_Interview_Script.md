...
| We solve Minimum Interval to Include Each Query. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Sort + Min-Heap as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(Q \times N) from source. | 來源暴力時間約 O(Q \times N)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Sort + Min-Heap. | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(N \log N + Q \log Q) from source. | 來源優化時間為 O(N \log N + Q \log Q)。 | Approach |
| Optimized space is O(N + Q) from source. | 來源優化空間為 O(N + Q)。 | Approach |
...
| Sample input: intervals = [[1,4],[2,4],[3,6],[4,4]], queries = [2,3,4,5]. | 範例輸入：intervals = [[1,4],[2,4],[3,6],[4,4]], queries = [2,3,4,5]。 | Dry-run |
...
| Expected output: [3,3,1,4]. | 預期輸出：[3,3,1,4]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(N \log N + Q \log Q). | 時間複雜度是 O(N \log N + Q \log Q)。 | Complexity |
| Space complexity is O(N + Q). | 空間複雜度是 O(N + Q)。 | Complexity |
...
| Main runtime from source is O(N \log N + Q \log Q). | 來源主要時間為 O(N \log N + Q \log Q)。 | Complexity |
| Extra memory from source is O(N + Q). | 來源額外空間為 O(N + Q)。 | Complexity |
...
| Time is O(N \log N + Q \log Q), space is O(N + Q). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(N \log N + Q \log Q). | 速記重點。 | Cheat sheet |
| Report space complexity O(N + Q). | 速記重點。 | Cheat sheet |
