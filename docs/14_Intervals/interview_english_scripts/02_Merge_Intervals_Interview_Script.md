...
| We solve Merge Intervals. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Sort & Merge as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Sort & Merge. | 優化法使用來源主方法。 | Approach |
...
| Sample input: intervals = [[1,3],[2,6],[8,10],[15,18]]. | 範例輸入：intervals = [[1,3],[2,6],[8,10],[15,18]]。 | Dry-run |
...
| Expected output: [[1,6],[8,10],[15,18]]. | 預期輸出：[[1,6],[8,10],[15,18]]。 | Dry-run |
