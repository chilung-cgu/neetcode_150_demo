...
| We solve Non-overlapping Intervals. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Greedy (Sort by Start) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Greedy (Sort by Start). | 優化法使用來源主方法。 | Approach |
...
| Sample input: intervals = [[1,2],[2,3],[3,4],[1,3]]. | 範例輸入：intervals = [[1,2],[2,3],[3,4],[1,3]]。 | Dry-run |
