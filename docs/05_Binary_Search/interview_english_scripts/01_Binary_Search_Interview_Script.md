...
| We solve Binary Search. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Iterative Binary Search as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Iterative Binary Search. | 優化法使用來源主方法。 | Approach |
...
| Sample input: nums = [-1,0,3,5,9,12], target = 9. | 範例輸入：nums = [-1,0,3,5,9,12], target = 9。 | Dry-run |
