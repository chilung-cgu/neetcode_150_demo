...
| We solve Reconstruct Itinerary. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Hierholzer's Algorithm as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Hierholzer's Algorithm. | 優化法使用來源主方法。 | Approach |
...
| Finally, I choose one option and update state. | 依來源步驟執行。 | Coding |
| I recurse to next decision level. | 依來源步驟執行。 | Coding |
...
| Sample input: [["MUC","LHR"],["JFK","MUC"],["SFO","SJC"],["LHR","SFO"]]. | 範例輸入：[["MUC","LHR"],["JFK","MUC"],["SFO","SJC"],["LHR","SFO"]]。 | Dry-run |
...
| Expected output: ["JFK","MUC","LHR","SFO","SJC"]. | 預期輸出：["JFK","MUC","LHR","SFO","SJC"]。 | Dry-run |
