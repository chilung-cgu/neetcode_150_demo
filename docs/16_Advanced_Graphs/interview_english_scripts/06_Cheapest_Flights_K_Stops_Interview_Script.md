...
| We solve Cheapest Flights Within K Stops. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Bellman-Ford as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(n^k) from source. | 來源暴力時間約 O(n^k)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Bellman-Ford. | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(K \times E) from source. | 來源優化時間為 O(K \times E)。 | Approach |
...
| Then, I relax outgoing edges and update distances. | 依來源步驟執行。 | Coding |
| Next, I push improved states into heap. | 依來源步驟執行。 | Coding |
| Finally, I return final shortest-path result. | 依來源步驟執行。 | Coding |
...
| Sample input: n = 3, flights = [[0,1,100],[1,2,100],[0,2,500]], src = 0, dst = 2, | 範例輸入：n = 3, flights = [[0,1,100],[1,2,100],[0,2,500]], src = 0, dst = 2, k = 1。 | Dry-run |
...
| Expected output: 200. | 預期輸出：200。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(K \times E). | 時間複雜度是 O(K \times E)。 | Complexity |
...
| Main runtime from source is O(K \times E). | 來源主要時間為 O(K \times E)。 | Complexity |
...
| Time is O(K \times E), space is O(N). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(K \times E). | 速記重點。 | Cheat sheet |
