...
| We solve Word Search II. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Backtracking with Trie as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(K \times M \times N \times 4^L) from source. | 來源暴力時間約 O(K \times M \times N \times 4^L)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Backtracking with Trie. | 優化法使用來源主方法。 | Approach |
...
| Optimized space is O(K \times L) from source. | 來源優化空間為 O(K \times L)。 | Approach |
...
| Finally, I return lookup or aggregate result. | 依來源步驟執行。 | Coding |
...
| Expected output: ["eat","oath"]. | 預期輸出：["eat","oath"]。 | Dry-run |
...
| Space complexity is O(K \times L). | 空間複雜度是 O(K \times L)。 | Complexity |
...
| Extra memory from source is O(K \times L). | 來源額外空間為 O(K \times L)。 | Complexity |
...
| Time is O(M \times N \times 4^L), space is O(K \times L). | 收尾時可用這句。 | Wrap-up |
...
| Report space complexity O(K \times L). | 速記重點。 | Cheat sheet |
