...
| We solve 3Sum. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Sorting + Two Pointers as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Sorting + Two Pointers. | 優化法使用來源主方法。 | Approach |
...
| Sample input: [-1,0,1,2,-1,-4]. | 範例輸入：[-1,0,1,2,-1,-4]。 | Dry-run |
...
| Expected output: [[-1,-1,2], [-1,0,1]]. | 預期輸出：[[-1,-1,2], [-1,0,1]]。 | Dry-run |
