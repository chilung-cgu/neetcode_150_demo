...
| We solve Generate Parentheses. | 我們要解這一題。 | Restatement |
...
| Brute time is about O(2^{2n} \cdot n) from source. | 來源暴力時間約 O(2^{2n} \cdot n)。 | Approach |
...
| Optimized time is O(\frac{4^n}{\sqrt{n}}) from source. | 來源優化時間為 O(\frac{4^n}{\sqrt{n}})。 | Approach |
...
| Sample input: n = 3. | 範例輸入：n = 3。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(\frac{4^n}{\sqrt{n}}). | 時間複雜度是 O(\frac{4^n}{\sqrt{n}})。 | Complexity |
...
| Main runtime from source is O(\frac{4^n}{\sqrt{n}}). | 來源主要時間為 O(\frac{4^n}{\sqrt{n}})。 | Complexity |
...
| Time is O(\frac{4^n}{\sqrt{n}}), space is O(n). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(\frac{4^n}{\sqrt{n}}). | 速記重點。 | Cheat sheet |
