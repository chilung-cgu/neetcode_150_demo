...
| We solve Daily Temperatures. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Monotonic Stack as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Monotonic Stack. | 優化法使用來源主方法。 | Approach |
...
| Sample input: temperatures = [73,74,75,71,69,72,76,73]. | 範例輸入：temperatures = [73,74,75,71,69,72,76,73]。 | Dry-run |
...
| Expected output: [1,1,4,2,1,1,0,0]. | 預期輸出：[1,1,4,2,1,1,0,0]。 | Dry-run |
