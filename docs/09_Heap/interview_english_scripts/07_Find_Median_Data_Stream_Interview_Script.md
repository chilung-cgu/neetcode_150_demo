...
| We solve Find Median from Data Stream. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Two Heaps as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Two Heaps. | 優化法使用來源主方法。 | Approach |
...
| Time is O(\log N), space is O(N). | 收尾時可用這句。 | Wrap-up |
