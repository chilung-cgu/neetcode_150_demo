...
| We solve Design Twitter. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Hash Map + Max-Heap (Merge K Sorted Lists) as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(TotalTweets) from source. | 來源暴力時間約 O(TotalTweets)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Hash Map + Max-Heap (Merge K Sorted Lists). | 優化法使用來源主方法。 | Approach |
...
| Optimized space is O(U + T) from source. | 來源優化空間為 O(U + T)。 | Approach |
...
| Space complexity is O(U + T). | 空間複雜度是 O(U + T)。 | Complexity |
...
| Extra memory from source is O(U + T). | 來源額外空間為 O(U + T)。 | Complexity |
...
| Time is O(1), space is O(U + T). | 收尾時可用這句。 | Wrap-up |
...
| Report space complexity O(U + T). | 速記重點。 | Cheat sheet |
