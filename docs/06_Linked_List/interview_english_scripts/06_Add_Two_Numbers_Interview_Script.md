...
| We solve Add Two Numbers. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Iterative as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Iterative. | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(\max(m, n) from source. | 來源優化時間為 O(\max(m, n)。 | Approach |
| Optimized space is O(\max(m, n) from source. | 來源優化空間為 O(\max(m, n)。 | Approach |
...
| Sample input: l1 = [2,4,3] (342), l2 = [5,6,4] (465). | 範例輸入：l1 = [2,4,3] (342), l2 = [5,6,4] (465)。 | Dry-run |
...
| Expected output: [7,0,8] (807). | 預期輸出：[7,0,8] (807)。 | Dry-run |
...
| Space complexity is O(\max(m, n). | 空間複雜度是 O(\max(m, n)。 | Complexity |
...
| Main runtime from source is O(\max(m, n). | 來源主要時間為 O(\max(m, n)。 | Complexity |
| Extra memory from source is O(\max(m, n). | 來源額外空間為 O(\max(m, n)。 | Complexity |
...
| Time is O(\max(m, n), space is O(\max(m, n). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(\max(m, n). | 速記重點。 | Cheat sheet |
| Report space complexity O(\max(m, n). | 速記重點。 | Cheat sheet |
