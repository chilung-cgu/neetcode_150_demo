...
| We solve LRU Cache. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Hash Map + Doubly Linked List as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Hash Map + Doubly Linked List. | 優化法使用來源主方法。 | Approach |
...
| Optimized space is O(capacity) from source. | 來源優化空間為 O(capacity)。 | Approach |
...
| Extra memory from source is O(capacity). | 來源額外空間為 O(capacity)。 | Complexity |
...
| Time is O(1), space is O(capacity). | 收尾時可用這句。 | Wrap-up |
...
| Report space complexity O(capacity). | 速記重點。 | Cheat sheet |
