...
| We solve Longest Increasing Subsequence. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use DP (O(N^2)) - Easier to implement/explain first as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses DP (O(N^2)) - Easier to implement/explain first. | 優化法使用來源主方法。 | Approach |
...
| Sample input: nums = [10,9,2,5,3,7,101,18]. | 範例輸入：nums = [10,9,2,5,3,7,101,18]。 | Dry-run |
