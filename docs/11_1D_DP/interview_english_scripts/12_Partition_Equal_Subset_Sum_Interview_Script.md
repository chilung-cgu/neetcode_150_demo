...
| We solve Partition Equal Subset Sum. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use DP with Set (Easy to understand) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses DP with Set (Easy to understand). | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(N \times S) from source. | 來源優化時間為 O(N \times S)。 | Approach |
| Optimized space is O(S) from source. | 來源優化空間為 O(S)。 | Approach |
...
| Finally, I return final dp target value. | 依來源步驟執行。 | Coding |
...
| Sample input: nums = [1,5,11,5]. | 範例輸入：nums = [1,5,11,5]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(N \times S). | 時間複雜度是 O(N \times S)。 | Complexity |
| Space complexity is O(S). | 空間複雜度是 O(S)。 | Complexity |
...
| Main runtime from source is O(N \times S). | 來源主要時間為 O(N \times S)。 | Complexity |
| Extra memory from source is O(S). | 來源額外空間為 O(S)。 | Complexity |
...
| Time is O(N \times S), space is O(S). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(N \times S). | 速記重點。 | Cheat sheet |
| Report space complexity O(S). | 速記重點。 | Cheat sheet |
