...
| We solve Product of Array Except Self. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Prefix & Postfix (Optimal) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Prefix & Postfix (Optimal). | 優化法使用來源主方法。 | Approach |
...
| Then, I maintain prefix state for left-side contribution. | 依來源步驟執行。 | Coding |
| Next, I combine postfix state for right-side contribution. | 依來源步驟執行。 | Coding |
...
| Finally, I return final result after loop. | 依來源步驟執行。 | Coding |
...
| Sample input: [1,2,3,4]. | 範例輸入：[1,2,3,4]。 | Dry-run |
...
| Expected output: [24,12,8,6]. | 預期輸出：[24,12,8,6]。 | Dry-run |
