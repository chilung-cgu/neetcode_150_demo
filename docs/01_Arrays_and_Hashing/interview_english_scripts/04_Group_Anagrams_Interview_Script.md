...
| We solve Group Anagrams. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Categorize by Sorted String (Recommended) as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(m^2 \cdot n) from source. | 來源暴力時間約 O(m^2 \cdot n)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Categorize by Sorted String (Recommended). | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(m \cdot n \log n) from source. | 來源優化時間為 O(m \cdot n \log n)。 | Approach |
| Optimized space is O(m \cdot n) from source. | 來源優化空間為 O(m \cdot n)。 | Approach |
...
| Finally, I return final computed integer result. | 依來源步驟執行。 | Coding |
...
| Sample input: ["eat", "tea", "tan", "ate", "nat", "bat"]. | 範例輸入：["eat", "tea", "tan", "ate", "nat", "bat"]。 | Dry-run |
...
| Expected output: [["bat"], ["nat","tan"], ["ate","eat","tea"]]. | 預期輸出：[["bat"], ["nat","tan"], ["ate","eat","tea"]]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(m \cdot n \log n). | 時間複雜度是 O(m \cdot n \log n)。 | Complexity |
| Space complexity is O(m \cdot n). | 空間複雜度是 O(m \cdot n)。 | Complexity |
...
| Main runtime from source is O(m \cdot n \log n). | 來源主要時間為 O(m \cdot n \log n)。 | Complexity |
| Extra memory from source is O(m \cdot n). | 來源額外空間為 O(m \cdot n)。 | Complexity |
...
| Time is O(m \cdot n \log n), space is O(m \cdot n). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(m \cdot n \log n). | 速記重點。 | Cheat sheet |
| Report space complexity O(m \cdot n). | 速記重點。 | Cheat sheet |
