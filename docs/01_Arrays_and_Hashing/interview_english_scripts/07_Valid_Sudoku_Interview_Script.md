...
| We solve Valid Sudoku. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use One Pass with Arrays as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(9^2) from source. | 來源暴力時間約 O(9^2)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses One Pass with Arrays. | 優化法使用來源主方法。 | Approach |
...
| Finally, I continue until boundaries meet. | 依來源步驟執行。 | Coding |
| I return boundary or candidate result. | 依來源步驟執行。 | Coding |
...
| Sample input: vector<vector<char>> board. | 範例輸入：vector<vector<char>> board。 | Dry-run |
