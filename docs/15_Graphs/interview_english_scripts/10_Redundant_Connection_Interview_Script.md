...
| We solve Redundant Connection. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Union-Find as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Union-Find. | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(N \alpha(N) from source. | 來源優化時間為 O(N \alpha(N)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| First, I initialize parent and rank arrays. | 依來源步驟執行。 | Coding |
| Next, I define find with path compression. | 依來源步驟執行。 | Coding |
...
| Then, I define union by rank rule. | 依來源步驟執行。 | Coding |
| Next, I process edges one by one. | 依來源步驟執行。 | Coding |
| Then, I detect cycle or connectivity conditions. | 依來源步驟執行。 | Coding |
| Next, I merge sets when union is valid. | 依來源步驟執行。 | Coding |
| Finally, I return final graph decision. | 依來源步驟執行。 | Coding |
...
| Sample input: edges = [[1,2], [1,3], [2,3]]. | 範例輸入：edges = [[1,2], [1,3], [2,3]]。 | Dry-run |
...
| Expected output: [2,3]. | 預期輸出：[2,3]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(N \alpha(N). | 時間複雜度是 O(N \alpha(N)。 | Complexity |
...
| Main runtime from source is O(N \alpha(N). | 來源主要時間為 O(N \alpha(N)。 | Complexity |
...
| Time is O(N \alpha(N), space is O(N). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(N \alpha(N). | 速記重點。 | Cheat sheet |
