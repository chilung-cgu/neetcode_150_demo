...
| We solve Clone Graph. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use DFS with Hash Map as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses DFS with Hash Map. | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(V + E) from source. | 來源優化時間為 O(V + E)。 | Approach |
| Optimized space is O(V) from source. | 來源優化空間為 O(V)。 | Approach |
...
| Finally, I undo choice to restore state. | 依來源步驟執行。 | Coding |
| I return all collected combinations. | 依來源步驟執行。 | Coding |
...
| Sample input: Adjacency list. | 範例輸入：Adjacency list。 | Dry-run |
...
| Expected output: Same adjacency list (but new objects).. | 預期輸出：Same adjacency list (but new objects).。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(V + E). | 時間複雜度是 O(V + E)。 | Complexity |
| Space complexity is O(V). | 空間複雜度是 O(V)。 | Complexity |
...
| Main runtime from source is O(V + E). | 來源主要時間為 O(V + E)。 | Complexity |
| Extra memory from source is O(V). | 來源額外空間為 O(V)。 | Complexity |
...
| Time is O(V + E), space is O(V). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(V + E). | 速記重點。 | Cheat sheet |
| Report space complexity O(V). | 速記重點。 | Cheat sheet |
