# 11 Number of Connected Components in an Undirected Graph — Interview English Script (C++)
...
| We solve Number of Connected Components in an Undirected Graph. | 我們要解這一題。 | Restatement |
...
| Brute time is about O(V + E) from source. | 來源暴力時間約 O(V + E)。 | Approach |
...
| Optimized time is O(E \alpha(N) from source. | 來源優化時間為 O(E \alpha(N)。 | Approach |
...
| Sample input: n = 5, edges = [[0,1], [1,2], [3,4]]. | 範例輸入：n = 5, edges = [[0,1], [1,2], [3,4]]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(E \alpha(N). | 時間複雜度是 O(E \alpha(N)。 | Complexity |
...
| Main runtime from source is O(E \alpha(N). | 來源主要時間為 O(E \alpha(N)。 | Complexity |
...
| Time is O(E \alpha(N), space is O(N). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(E \alpha(N). | 速記重點。 | Cheat sheet |
