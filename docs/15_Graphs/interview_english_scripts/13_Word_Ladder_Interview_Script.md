...
| We solve Word Ladder. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use BFS with Set as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(M \times L \times 26) from source. | 來源暴力時間約 O(M \times L \times 26)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses BFS with Set. | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(M \times L \times 26) from source. | 來源優化時間為 O(M \times L \times 26)。 | Approach |
| Optimized space is O(M \times L) from source. | 來源優化空間為 O(M \times L)。 | Approach |
...
| Finally, I update answer during traversal. | 依來源步驟執行。 | Coding |
| I return final graph result. | 依來源步驟執行。 | Coding |
...
| Sample input: beginWord = "hit", endWord = "cog", wordList = ["hot","dot","dog","lot","log","cog"]. | 範例輸入：beginWord = "hit", endWord = "cog", wordList = ["hot","dot","dog","lot","log","cog"]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(M \times L \times 26). | 時間複雜度是 O(M \times L \times 26)。 | Complexity |
| Space complexity is O(M \times L). | 空間複雜度是 O(M \times L)。 | Complexity |
...
| Main runtime from source is O(M \times L \times 26). | 來源主要時間為 O(M \times L \times 26)。 | Complexity |
| Extra memory from source is O(M \times L). | 來源額外空間為 O(M \times L)。 | Complexity |
...
| Time is O(M \times L \times 26), space is O(M \times L). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(M \times L \times 26). | 速記重點。 | Cheat sheet |
| Report space complexity O(M \times L). | 速記重點。 | Cheat sheet |
