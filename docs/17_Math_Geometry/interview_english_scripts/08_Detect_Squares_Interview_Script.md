...
| We solve Detect Squares. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Hash Map Counting as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Hash Map Counting. | 優化法使用來源主方法。 | Approach |
...
| Optimized space is O(N + 1000^2) from source. | 來源優化空間為 O(N + 1000^2)。 | Approach |
...
| Space complexity is O(N + 1000^2). | 空間複雜度是 O(N + 1000^2)。 | Complexity |
...
| Extra memory from source is O(N + 1000^2). | 來源額外空間為 O(N + 1000^2)。 | Complexity |
...
| Time is O(1), space is O(N + 1000^2). | 收尾時可用這句。 | Wrap-up |
...
| Report space complexity O(N + 1000^2). | 速記重點。 | Cheat sheet |
