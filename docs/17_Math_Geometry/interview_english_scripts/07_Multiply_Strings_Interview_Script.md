...
| We solve Multiply Strings. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Optimized Simulation as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Optimized Simulation. | 優化法使用來源主方法。 | Approach |
...
| Optimized space is O(M + N) from source. | 來源優化空間為 O(M + N)。 | Approach |
...
| Sample input: num1 = "2", num2 = "3". | 範例輸入：num1 = "2", num2 = "3"。 | Dry-run |
...
| Space complexity is O(M + N). | 空間複雜度是 O(M + N)。 | Complexity |
...
| Extra memory from source is O(M + N). | 來源額外空間為 O(M + N)。 | Complexity |
...
| Time is O(M \times N), space is O(M + N). | 收尾時可用這句。 | Wrap-up |
...
| Report space complexity O(M + N). | 速記重點。 | Cheat sheet |
