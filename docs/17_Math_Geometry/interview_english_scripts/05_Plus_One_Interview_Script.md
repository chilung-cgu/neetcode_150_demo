...
| We solve Plus One. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Iterate Backwards as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Iterate Backwards. | 優化法使用來源主方法。 | Approach |
...
| Sample input: digits = [1,2,3]. | 範例輸入：digits = [1,2,3]。 | Dry-run |
...
| Expected output: [1,2,4]. | 預期輸出：[1,2,4]。 | Dry-run |
