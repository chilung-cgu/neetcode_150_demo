...
| We solve Set Matrix Zeroes. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use O(1) Space as main method. | 我會用來源主方法。 | Restatement |
...
| Brute space is about O(m+n) from source. | 來源暴力空間約 O(m+n)。 | Approach |

### Optimized approach (5 lines)

| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses O(1) Space. | 優化法使用來源主方法。 | Approach |
