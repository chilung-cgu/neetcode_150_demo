...
| We solve Palindrome Partitioning. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Backtracking with Naive Palindrome Check as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(N \times 2^N) from source. | 來源暴力時間約 O(N \times 2^N)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Backtracking with Naive Palindrome Check. | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(N \cdot 2^N) from source. | 來源優化時間為 O(N \cdot 2^N)。 | Approach |
...
| Sample input: s = "aab". | 範例輸入：s = "aab"。 | Dry-run |
...
| Expected output: [["a","a","b"],["aa","b"]]. | 預期輸出：[["a","a","b"],["aa","b"]]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(N \cdot 2^N). | 時間複雜度是 O(N \cdot 2^N)。 | Complexity |
...
| Main runtime from source is O(N \cdot 2^N). | 來源主要時間為 O(N \cdot 2^N)。 | Complexity |
...
| Time is O(N \cdot 2^N), space is O(N). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(N \cdot 2^N). | 速記重點。 | Cheat sheet |
