...
| We solve N-Queens. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Backtracking with Sets as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Backtracking with Sets. | 優化法使用來源主方法。 | Approach |
...
| Sample input: n = 4. | 範例輸入：n = 4。 | Dry-run |
