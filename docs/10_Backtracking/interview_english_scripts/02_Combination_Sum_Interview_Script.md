...
| We solve Combination Sum. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Backtracking (DFS) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Backtracking (DFS). | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(N^{\frac{T}{M}}) from source. | 來源優化時間為 O(N^{\frac{T}{M}})。 | Approach |
| Optimized space is O(T/M) from source. | 來源優化空間為 O(T/M)。 | Approach |
...
| Then, I sort data when source logic requires order. | 依來源步驟執行。 | Coding |
...
| Sample input: candidates = [2,3,6,7], target = 7. | 範例輸入：candidates = [2,3,6,7], target = 7。 | Dry-run |
...
| Expected output: [[2,2,3],[7]]. | 預期輸出：[[2,2,3],[7]]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(N^{\frac{T}{M}}). | 時間複雜度是 O(N^{\frac{T}{M}})。 | Complexity |
| Space complexity is O(T/M). | 空間複雜度是 O(T/M)。 | Complexity |
...
| Main runtime from source is O(N^{\frac{T}{M}}). | 來源主要時間為 O(N^{\frac{T}{M}})。 | Complexity |
| Extra memory from source is O(T/M). | 來源額外空間為 O(T/M)。 | Complexity |
...
| Time is O(N^{\frac{T}{M}}), space is O(T/M). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(N^{\frac{T}{M}}). | 速記重點。 | Cheat sheet |
| Report space complexity O(T/M). | 速記重點。 | Cheat sheet |
