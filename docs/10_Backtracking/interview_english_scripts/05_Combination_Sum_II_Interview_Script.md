...
| We solve Combination Sum II. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Backtracking (With Sorting & Pruning) as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(2^N \times N) from source. | 來源暴力時間約 O(2^N \times N)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Backtracking (With Sorting & Pruning). | 優化法使用來源主方法。 | Approach |
| I keep key invariant during updates. | 更新時維持關鍵不變量。 | Approach |
| I apply source transitions step by step. | 依來源規則逐步轉移。 | Approach |
| Optimized time is O(2^N) from source. | 來源優化時間為 O(2^N)。 | Approach |
...
| Sample input: candidates = [10,1,2,7,6,1,5], target = 8. | 範例輸入：candidates = [10,1,2,7,6,1,5], target = 8。 | Dry-run |
...
| Expected output: [[1,1,6],[1,2,5],[1,7],[2,6]]. | 預期輸出：[[1,1,6],[1,2,5],[1,7],[2,6]]。 | Dry-run |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Time complexity is O(2^N). | 時間複雜度是 O(2^N)。 | Complexity |
...
| Main runtime from source is O(2^N). | 來源主要時間為 O(2^N)。 | Complexity |
...
| Time is O(2^N), space is O(N). | 收尾時可用這句。 | Wrap-up |
...
| Report time complexity O(2^N). | 速記重點。 | Cheat sheet |
