...
| We solve Longest Substring Without Repeating Characters. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Sliding Window + Hash Set as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Sliding Window + Hash Set. | 優化法使用來源主方法。 | Approach |
...
| Optimized space is O(min(n, m) from source. | 來源優化空間為 O(min(n, m)。 | Approach |
...
| Finally, I return the best recorded answer. | 依來源步驟執行。 | Coding |
...
| Sample input: "abcabcbb". | 範例輸入："abcabcbb"。 | Dry-run |
...
| Space complexity is O(min(n, m). | 空間複雜度是 O(min(n, m)。 | Complexity |
...
| Extra memory from source is O(min(n, m). | 來源額外空間為 O(min(n, m)。 | Complexity |
...
| Time is O(n), space is O(min(n, m). | 收尾時可用這句。 | Wrap-up |
...
| Report space complexity O(min(n, m). | 速記重點。 | Cheat sheet |
