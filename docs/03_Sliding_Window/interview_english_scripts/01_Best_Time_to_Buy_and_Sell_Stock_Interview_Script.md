...
| We solve Best Time to Buy and Sell Stock. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Linear Scan (One Pass) as main method. | 我會用來源主方法。 | Restatement |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Linear Scan (One Pass). | 優化法使用來源主方法。 | Approach |
...
| Sample input: [7,1,5,3,6,4]. | 範例輸入：[7,1,5,3,6,4]。 | Dry-run |
