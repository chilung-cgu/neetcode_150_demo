...
| We solve Minimum Window Substring. | 我們要解這一題。 | Restatement |
| Input and output follow the source statement. | 輸入輸出依來源敘述。 | Restatement |
| I will use Optimized Sliding Window as main method. | 我會用來源主方法。 | Restatement |
...
| Brute time is about O(n^3) from source. | 來源暴力時間約 O(n^3)。 | Approach |
...
| English line | Traditional Chinese meaning (short) | Interview stage |
|---|---|---|
| Optimized method uses Optimized Sliding Window. | 優化法使用來源主方法。 | Approach |
...
| Next, I update counts or score for new item. | 依來源步驟執行。 | Coding |
| Then, I shrink window when condition is invalid. | 依來源步驟執行。 | Coding |
| Next, I update best answer during valid window. | 依來源步驟執行。 | Coding |
| Then, I continue until right reaches end. | 依來源步驟執行。 | Coding |
| Next, I return the best recorded answer. | 依來源步驟執行。 | Coding |
...
| Sample input: s = "ADOBECODEBANC", t = "ABC". | 範例輸入：s = "ADOBECODEBANC", t = "ABC"。 | Dry-run |
...
| Expected output: "BANC". | 預期輸出："BANC"。 | Dry-run |
